...
export default async function editNote(noteId: string, _prevState: unknown, formData: FormData): Promise<EditNoteFormActionResult> {
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      await updateNote(noteId, userId, { title, content }, tx);

      // Delete all note chunks for a note and the current user
      await deleteNoteChunks(userId, noteId, tx);

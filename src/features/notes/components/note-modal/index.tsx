...
export default function NoteModal({ icon, browseBar, noteTitle, children, className, ...props }: NoteModalProps) {
  ...
  return (
    ...
    >
      <div className="bg-background grid max-h-[min(95dvb,100%)] max-w-[min(96ch,100%)] grid-rows-[auto_1fr] items-start overflow-hidden">
        <Header icon={icon} title={noteTitle ?? "New Note"} onClosed={() => dialogRef.current?.close()} />
        <Content>
          {browseBar}
          {children}
        </Content>

...
export class Note extends Effect.Service<Note>()("Note", {
  ...
  effect: Effect.gen(function* () {
    ...
    const getNotesWithPagination = (
      userId: string,
      searchTerm: string = "",
      currentPage: number = 1,
      itemsPerPage: number = 10,
      sortByField: SBF = "updated_at",
      sortByDirection: SBD = "desc",
    ...
    const getMostRecentNotes = (userId: string, limit?: number) =>
      execute((dbOrTx) => {
        const query = dbOrTx
          .select({
            id: NoteTable.id,
    ...
    const getNote = (id: string, userId: string) =>
      execute((dbOrTx) =>
    ...
    const getNoteTitle = (id: string, userId: string) =>
    ...
    const getNotePreferences = (id: string, userId: string) =>
    ...
    const deleteNotePreferences = (id: string, userId: string) =>
      execute((dbOrTx) =>
        dbOrTx
          .update(NoteTable)
    ...
    const insertNote = (userId: string, data: Omit<typeof NoteTable.$inferInsert, "userId">) =>
      execute((dbOrTx) =>
        dbOrTx
          .insert(NoteTable)
          .values({ userId, ...data })
    ...
    const updateNote = (id: string, userId: string, data: Partial<Omit<typeof NoteTable.$inferInsert, "id" | "userId">>) =>
      execute((dbOrTx) =>
        dbOrTx
          .update(NoteTable)
          .set(data)
    ...
    const deleteNote = (id: string, userId: string) =>
      execute((dbOrTx) => dbOrTx.delete(NoteTable).where(and(eq(NoteTable.id, id), eq(NoteTable.userId, userId))));

...
2.  **Reasoning and Inference:** If the user's query implies a task or asks for a suggestion based on their notes (e.g., "What should I focus on today?", "Should I buy chicken breast?"), you **MUST** first use the \`getInformation\` tool to find relevant notes (like a to-do list or a grocery list). Then, use the retrieved information to provide a helpful, reasoned suggestion.
3.  **Direct Knowledge Retrieval:** For all other questions that are clearly about finding information within the notes, use the \`getInformation\` tool. Synthesize the information from the retrieved chunks to provide a direct and factual answer.

## Tool Usage and Response Formatting
- **Tool First:** Except for small talk, always use the \`getInformation\` tool before answering.
...
] as const satisfies NotesAssistantUIMessage[];

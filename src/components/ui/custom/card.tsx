...
function Card({ className, ...props }: React.ComponentProps<"article">) {
  return (
    <article
      data-slot="card"
      className={cn("bg-card text-card-foreground mx-auto flex w-full max-w-xl flex-col gap-6 rounded-xl border py-6 shadow-sm", className)}
      {...props}
    />
  );
}
...
function CardTitle({ className, ...props }: React.ComponentProps<"h2">) {
  return <h2 data-slot="card-title" className={cn("font-sans text-3xl leading-none uppercase", className)} {...props} />;
}
